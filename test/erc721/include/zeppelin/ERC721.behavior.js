...
function shouldBehaveLikeERC721 (errorPrefix, owner, newOwner, approved, anotherApproved, operator, other, burnable = true) {
  ...
  context('with minted tokens', function () {
    ...
    describe('balanceOf', function () {
      ...
      context('when querying the zero address', function () {
        it('throws', async function () {
          await expectRevert(
            this.token.balanceOf(ZERO_ADDRESS), 'zero address',
          );
        });
      });
    });
    ...
    describe('ownerOf', function () {
      ...
      context('when the given token ID was not tracked by this token', function () {
        ...
        it('reverts', async function () {
          await expectRevert(
            this.token.ownerOf(tokenId), "token doesn't exist",
          );
        });
      });
    });
    ...
    describe('transfers', function () {
      ...
      const transferWasSuccessful = function ({ owner, tokenId, approved }) {
        ...
        it('emits a Transfer event', async function () {
          expectEvent.inLogs(logs, 'Transfer', { _from: owner, _to: this.toWhom, _tokenId: tokenId });
        });
        ...
        it('emits an Approval event', async function () {
          expectEvent.inLogs(logs, 'Approval', { _owner: owner, _approved: ZERO_ADDRESS, _tokenId: tokenId });
        });
        ...
        context('to a receiver contract returning unexpected value', function () {
          it('reverts', async function () {
            ...
            await expectRevert(
              ...
              'invalid onERC721Received response', // 'ERC721: transfer to non ERC721Receiver implementer',
            );
          });
        });
    ...
    describe('approve', function () {
      ...
      context('when the address that receives the approval is the owner', function () {
        it('reverts', async function () {
          await expectRevert(
            this.token.approve(owner, tokenId, { from: owner }), 'self approval',
          );
        });
      });
  ...
  describe('_mint(address, uint256)', function () {
    it('reverts with a null destination address', async function () {
      await expectRevert(
        this.token.mint(ZERO_ADDRESS, firstTokenId), 'zero address',
      );
    });
    ...
    context('with minted token', async function () {
      ...
      it('emits a Transfer event', function () {
        expectEvent.inLogs(this.logs, 'Transfer', { _from: ZERO_ADDRESS, _to: owner, _tokenId: firstTokenId });
      });
      ...
      it('reverts when adding a token id that already exists', async function () {
        await expectRevert(this.token.mint(owner, firstTokenId), 'already minted');
      });
    });
  });
  ...
  if(burnable) describe('_burn', function () {
    it('reverts when burning a non-existent token id', async function () {
      await expectRevert(
        this.token.burn(firstTokenId), "token doesn't exist",
      );
    });
    ...
    context('with minted tokens', function () {
      ...
      context('with burnt token', function () {
        ...
        it('emits a Transfer event', function () {
          expectEvent.inLogs(this.logs, 'Transfer', { _from: owner, _to: ZERO_ADDRESS, _tokenId: firstTokenId });
        });

        it('emits an Approval event', function () {
          expectEvent.inLogs(this.logs, 'Approval', { _owner: owner, _approved: ZERO_ADDRESS, _tokenId: firstTokenId });
        });

        it('deletes the token', async function () {
          expect(await this.token.balanceOf(owner)).to.be.bignumber.equal('1');
          await expectRevert(
            this.token.ownerOf(firstTokenId), "token doesn't exist",
          );
        });
    ...
    context('with minted tokens', function () {
      ...
      context('with burnt token', function () {
        ...
        it('burns all tokens', async function () {
          ...
          await expectRevert(
            this.token.tokenByIndex(0), 'index out of bounds',
          );
        });
      });
    });
