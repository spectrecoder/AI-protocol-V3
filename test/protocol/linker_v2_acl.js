// Alethea iNFT Linker v2: features/roles (ACL) Tests
...
// run iNFT Linker v2 features/roles (ACL) tests
contract("iNFT Linker v2: features/roles (ACL) tests", function(accounts) {
	...
	beforeEach(async function() {
		({ali, persona, iNft, linker} = await linker_v2_deploy_restricted(a0));
		// initialize linker v2 with the v1 defaults
	...
	describe("when iNFT exists (already linked)", function() {
		...
		describe("when FEATURE_UNLINKING and FEATURE_ALLOW_ANY_NFT_CONTRACT are enabled", function() {
			beforeEach(async function() {
				await linker.updateFeatures(FEATURE_UNLINKING | FEATURE_ALLOW_ANY_NFT_CONTRACT, {from: a0});
			});
	...
	async function whitelistTargetContract() {
		return await linker.whitelistTargetContract(new_nft.address, true, true, {from: by});
	}
	...
	describe("when sender has ROLE_WHITELIST_MANAGER permission", function() {
		...
		describe("whitelistTargetContract succeeds", function() {
			...
			it("whitelistTargetContract gets set as expected", async function() {
				expect(await linker.whitelistedTargetContracts(new_nft.address)).to.be.bignumber.that.equals("3");
			});
