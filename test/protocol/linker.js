...
// run iNFT Linker tests
contract("iNFT Linker: tests", function(accounts) {
	...
	const NEXT_ID = new BN(LINKER_PARAMS.NEXT_ID);

	describe("iNFT Linker deployment", function() {
		...
		it("fails if ALI address is not set", async function() {
			...
			await expectRevert(
				linker_deploy_pure(a0, ali_addr, persona_addr, iNft_addr),
		...
		describe("succeeds with valid ALI, AI Personality, and iNFT", function() {
			let linker;
			beforeEach(async function() {
				linker = await linker_deploy_pure(a0, ali_addr, persona_addr, iNft_addr);
			});
			...
			it("linking price is as expected", async function() {
				expect(await linker.linkPrice()).to.be.bignumber.that.equals(LINKER_PARAMS.LINK_PRICE);
			});
			it("linking fee is as expected", async function() {
				expect(await linker.linkFee()).to.be.bignumber.that.equals("0");
			});
			...
			it("no whitelisted contract exists", async function() {
				expect(await linker.whitelistedTargetContracts(nft.address)).to.be.false;
			});
		});
	});
	describe("after iNFT Linker is deployed", function() {
		...
		beforeEach(async function() {
			({ali, persona, iNft, linker} = await linker_deploy(a0));
		...
		describe("whitelisting the target NFT smart contract", function() {
			it("fails if address to whitelist is zero", async function() {
				await expectRevert(linker.whitelistTargetContract(ZERO_ADDRESS, true, {from: a0}), "zero address");
			});
			it("fails if address to whitelist is not a ERC721", async function() {
				await expectRevert(linker.whitelistTargetContract(ali.address, true, {from: a0}), "target NFT is not ERC721");
			});
			describe("succeeds otherwise", function() {
				let receipt;
				beforeEach(async function() {
					receipt = await linker.whitelistTargetContract(nft.address, true, {from: a0});
				});
				it("specified address gets whitelisted", async function() {
					expect(await linker.whitelistedTargetContracts(nft.address)).to.be.true;
				});
		...
		describe("removing target NFT smart contract from the whitelist", function() {
			beforeEach(async function() {
				await linker.whitelistTargetContract(nft.address, true, {from: a0});
			});
			it("fails if address to remove is zero", async function() {
				await expectRevert(linker.whitelistTargetContract(ZERO_ADDRESS, false, {from: a0}), "zero address");
			});
			it("succeeds if address to remove is ERC721", async function() {
				const receipt = await linker.whitelistTargetContract(persona.address, false, {from: a0});
				expectEvent(receipt, "TargetContractWhitelisted", {
					_by: a0,
			...
			it("succeeds if address to remove is not ERC721", async function() {
				const receipt = await linker.whitelistTargetContract(ali.address, false, {from: a0});
				expectEvent(receipt, "TargetContractWhitelisted", {
					_by: a0,
			...
			describe("succeeds if address to remove is ERC721 whitelisted previously", function() {
				let receipt;
				beforeEach(async function() {
					receipt = await linker.whitelistTargetContract(nft.address, false, {from: a0});
				});
			...
			describe("iNFT minted with removed NFT contract can still be unlinked", function() {
				beforeEach(async function() {
					...
					await linker.whitelistTargetContract(nft.address, false, {from: a0});
				});
		...
		describe("when iNFT was previously created", function() {
			...
			describe("unlinking the iNFT", function() {
				...
				describe("succeeds when executed by the owner of iNFT", function() {
					beforeEach(async function() {
						receipt = await linker.unlink(recordId, {from: nft_owner});
					});
					check_burn_succeeds();
			...
			describe("unlinking the NFT", function() {
				...
				describe("succeeds when executed by the owner of NFT", function() {
					beforeEach(async function() {
						receipt = await linker.unlinkNFT(nft.address, target_id, {from: nft_owner});
					});
					check_burn_succeeds();
