...
function shouldHaveBasicProps(name, symbol, decimals) {
...
function shouldHaveAtomicApprove(errorPrefix, initialSupply, initialHolder, recipient) {
        ...
        describe('when there was no approved amount before', function () {
          it('reverts', async function () {
            await expectRevert(this.token.decreaseAllowance(
              spender, amount, { from: initialHolder }), `${errorPrefix}decreased allowance below zero`,
            );
          });
        });
        ...
        describe('when the spender had an approved amount', function () {
          ...
          it('reverts when more than the full allowance is removed', async function () {
            await expectRevert(
              ...
              `${errorPrefix}decreased allowance below zero`,
            );
          });
        });
      }
  ...
  describe('increase allowance', function () {
    ...
    describe('when the spender is the zero address', function () {
      ...
      it('reverts', async function () {
        await expectRevert(
          this.token.increaseAllowance(spender, amount, { from: initialHolder }), `${errorPrefix}approve to the zero address`,
        );
      });
    });
  });
...
function shouldHaveMint (errorPrefix, initialSupply, initialHolder, recipient) {
  describe('_mint', function () {
    const amount = new BN(50);
    it('rejects a null account', async function () {
      await expectRevert(
        this.token.mint(ZERO_ADDRESS, amount), `${errorPrefix}zero address`,
      );
    });
...
function shouldHaveBurn (errorPrefix, initialSupply, initialHolder) {
    ...
    describe('for a non zero account', function () {
      it('rejects burning more than balance', async function () {
        await expectRevert(this.token.burn(
          initialHolder, initialSupply.addn(1)), `${errorPrefix}burn amount exceeds balance`,
        );
      });
